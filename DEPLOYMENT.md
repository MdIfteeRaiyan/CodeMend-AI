...
After deployment, the page footer must show `v4.0`.
...
The current v4.0 workspace is safe to host as a **product preview**. It includes a server-side adapter for an external isolated runner and falls back to clearly labelled guided behavior when that runner is not configured. It never compiles arbitrary code inside Vercel.
